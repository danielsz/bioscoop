...
sequence. ~body~ is a DSL expression evaluated repeatedly for each value
of the binding, with the result merged into a single filtergraph. All
of Clojure's arithmetic and string functions are available inside the
body.
