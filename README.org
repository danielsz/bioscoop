...
(defn filtergraph [] (to-ffmpeg (bioscoop (compose [formatting ["i0"] ["i1"]]
                                                    (n-fun 2)
                                                    [["o0"] ["o1"] blending]))))
#+end_src

The following bioscoop program uses a ~let~ binding but is otherwise equivalent:

#+begin_src clojure
(def alternate #(to-ffmpeg (bioscoop (let [formatting (chain (format {:pix_fmts "gbrp10"})
                                                             (split {:outputs 2}))
...
Presentation of photography work with the Ken Burns effect. 
